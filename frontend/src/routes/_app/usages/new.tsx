...
import { salesListOptions, salesDetailOptions } from '@/hooks/useSales'
...
const searchSchema = z.object({
  ...
  // 类别预设(业务目录 ?category= 入口):用于筛选记录,后端缺 content_category_id 单值过滤前仅作提示
...
function UsageNewPage() {
  ...
  const form = useForm<UsageFormValues>({
    ...
    defaultValues: {
      sales_record_id: search.salesRecordId ?? '',
    ...
    createMutation.mutate(
      ...
      },
      {
        onSuccess: (outcome) => {
          ...
          void navigate({ to: '/sales/$salesId', params: { salesId: values.sales_record_id } })
        },
  ...
  return (
    <form className="mx-auto flex max-w-xl flex-col gap-4" onSubmit={submit}>
      ...
      {search.salesRecordId ? (
        <LockedRecordCard salesRecordId={search.salesRecordId} />
      ) : (
        <RecordSelector
...
/** ?salesRecordId= 预选:锁定记录并显示剩余次数(设计 §7)。 */
    ...
    <Card>
      <CardContent className="flex flex-col gap-1 py-3 text-sm">
        {record ? (
          <>
            <p>
              {customerNameCell(record.customer_id)} · {formatDate(record.sale_date)}
            </p>
